...
void pose_estimation_2d2d(const std::vector<KeyPoint> &keypoints_1, const std::vector<KeyPoint> &keypoints_2,
                          const std::vector<DMatch> &matches, Mat &R, Mat &t, Mat &R_H, Mat &t_H);
...
int main(int argc, char **argv) {
    ...
    Mat R, t;
    Mat R_H, t_H;
    pose_estimation_2d2d(keypoints_1, keypoints_2, matches, R, t, R_H, t_H);

    //-- 三角化
    vector<Point3d> points;
    vector<Point3d> points_H;

    triangulation(keypoints_1, keypoints_2, matches, R, t, points);
    triangulation(keypoints_1, keypoints_2, matches, R_H, t_H, points_H);
    //-- 验证三角化点与特征点的重投影关系
    Mat K = (Mat_<double>(3, 3) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1);
    Mat img1_plot = img_1.clone();
    Mat img1_plot_H = img_1.clone();
    Mat img2_plot = img_2.clone();
    Mat img2_plot_H = img_2.clone();
    for (int i = 0; i < matches.size(); i++) {
        // 第一个图
        float depth1 = points[i].z;
        float depth1_H = points_H[i].z;
        cout << "depth: " << depth1 << endl;
        cout << "depth_H: " << depth1_H << endl;
...
void pose_estimation_2d2d(const std::vector<KeyPoint> &keypoints_1, const std::vector<KeyPoint> &keypoints_2,
    ...
    for (int i = 0; i < solutions; ++i) {
        ...
        // cout << t_h.at(i) << endl;
        triangulation(keypoints_1, keypoints_2, matches, R_h.at(i), t_h.at(i), points);
    ...
    cout << "========Best========" << endl;
    cout << "rotation" << best_idx << " = " << endl;
    cout << R_h.at(best_idx) << endl;
    cout << "translation" << best_idx << " = " << endl;
    cout << t_h.at(best_idx) << endl;
    R_H = R_h.at(best_idx);
    t_H = t_h.at(best_idx);
}
